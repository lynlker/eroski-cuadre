...
const inputNote = ["200eurinput", "100eurinput", "50eurinput", "20eurinput", "10eurinput", "5eurinput"];
const inputCoin = ["2eurinput", "1eurinput", "50ceninput", "20ceninput", "10ceninput", "5ceninput", "2ceninput", "1ceninput"];
const inputMods = ["2rulinput", "1rulinput", "50rulinput", "20rulinput", "10rulinput", "5rulinput", "2cenrulinput", "1cenrulinput"];
...
function tillCalc(){
    ...
    let moneyTotal = (eur.reduce((partialSum, a) => partialSum + a, 0) + bil.reduce((partialSum, a) => partialSum + a, 0));
    let dif1 = moneyTotal - 300;
    let dif2 = dif1

    // Aquí se calcula la diferencia y los billetes/monedas a entregar.
    if (moneyTotal < 300) {
        boxTill.innerHTML = `Tienes, en total, ${moneyTotal.toFixed(2)}€. Tienes menos de 300€! Esto no debería pasar... No te habrás olvidado de algo?<br>`;
        importeCaja.value = dif2.toFixed(2);
    } else if (moneyTotal === 300) {
        boxTill.innerHTML = `Tienes exactamente 300€! Qué suerte... No deberías entregar nada de caja, supongo.<br>`;
        importeCaja.value = dif2.toFixed(2);
    } else {
        boxTill.innerHTML = `Tienes, en total, ${moneyTotal.toFixed(2)}€. Lo siguiente sería entregar ${dif1.toFixed(2)}€ al mando. <br>
        ...
        for (i = 0; i < 6; i++){
            ...
            if (bilAmt[i] === 0 || bilAmt[i] === "0" || bilAmt[i] === ""){
                ...
                boxTill.insertAdjacentHTML("beforeend", `- ${bilAmt[i]} billete de ${noteName[i]}.<br>`);
            } else{
                boxTill.insertAdjacentHTML("beforeend", `- ${bilAmt[i]} billetes de ${noteName[i]}.<br>`);
            }
        }
        ...
        for (i = 0; i < 8; i++){
            ...
            if (coinAmt[i] === 0 || coinAmt[i] === "0" || coinAmt[i] === ""){
                ...
                boxTill.insertAdjacentHTML("beforeend", `- ${coinAmt[i]} moneda de ${coinName[i]}.<br>`);
            } else{
                boxTill.insertAdjacentHTML("beforeend", `- ${coinAmt[i]} monedas de ${coinName[i]}.<br>`);
            }
        }
        importeCaja.value = dif2.toFixed(2);
    }
...
function finalCalc(){
    if (importeCaja.value === 0 || importeCaja.value === "0" || importeCaja.value === "" || importeVentaNeta.value === 0 || 
    importeVentaNeta.value === "0" || importeVentaNeta.value === "" || importeImp.value === "" || importeImp.value === 0 || importeImp.value === "0"){
        alert("Por favor, asegúrate de que tanto el importe Caja como el importe Venta Neta no estén vacíos y que ambos sean mayores que 0.");
    } else{
